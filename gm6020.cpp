...
gm6020::gm6020(CAN &can,bool ctrl_type,int motor_num)
    : _can(can),_ctrl_type(ctrl_type),_motor_num(motor_num){
    if(_ctrl_type){
...
int gm6020::rbms_send(int* motor) {//motorへ制御信号を送信する関数
...
float gm6020::pid(float T,short rpm_now, short set_speed,float *delta_rpm_pre,float *ie,float KP, float KI,float KD )//pid制御
{
    float de;
    float delta_rpm;
...
void gm6020::spd_control(int* set_speed,int* motor, float KP, float KI, float KD){//速度制御用関数
    ...
    while(1){
        for(int id=0;id<_motor_num;id++){
            if(_msg.id==0x205+id){//esc idごとに受信データ割り振り
                ...
                if(_ctrl_type){
                    motor[id] = (int)pid(tm[id].read(),speed[id]/19,set_speed[id],&delta_rpm_pre[id],&ie[id], KP, KI, KD);
                }else{
